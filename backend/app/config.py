...
#
# 이 동역학에서 3배는 사실상 안 닿는다 — 랜덤워크 + 평균회귀는 기댓값이 0 이라
# 수익이 뉴스 램프를 타이밍 맞춰 먹는 데서만 나오고 왕복 수수료가 깎는다.
# 그래서 마감 시각이 실질적인 결승선이고, 이 배수는 "대박 나면 즉시 승리" 다.
TARGET_MULTIPLIER = 3

# 경주 길이. 마감에 도달하면 전원 청산하고 현금 1위가 이긴다.
#
# 측정으로 고른 값이다. 10분이면 승자가 판마다 갈리고(정소장 5 / 강사원 3 /
# 한실장 2) 1위와 꼴찌 격차가 1.2배로 접전이다. 더 짧으면 운이 커지고,
# 더 길면 통찰력 높은 쪽이 단조롭게 이긴다.
RACE_SECONDS = 600
