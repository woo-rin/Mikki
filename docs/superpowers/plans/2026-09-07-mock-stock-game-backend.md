...
> **구현 중 확정된 변경 (커밋 `bb53099` 가 최종)**: 아래 코드 블록은 두 곳이 낡았다.
> (1) `analyze` 가 세션 락을 쥔 채 Claude 호출을 기다리면 안 된다. 예산 차감까지만 락
> 안에서 하고, 호출은 락 밖에서 기다린 뒤 락을 다시 잡아 결과를 커밋하며 남은 램프를
> 그 시점으로 다시 센다. 락을 쥔 채 기다리면 같은 세션의 폴링이 막혀 시장이 얼어붙고,
> "기다리는 몇 초가 분석의 실질 비용" 이라는 스펙 규칙이 무효가 된다.
> (2) `engine.advance` 는 기여 가능한 계획만 한 번 추린 뒤 tick 루프를 돈다(Task 2 절 참조).
> 또 `pytest.ini` 를 추가해 경고를 기본으로 오류 취급하고, starlette/anyio 버전 스큐
> 한 건만 예외 처리한다.
...
Run: `.venv/bin/pytest tests/test_api.py -v`
Expected: PASS — 20 passed (계획 산문의 옛 수치는 오산이었다)
...
- `pytest` 120건이 `ANTHROPIC_API_KEY` 없이 통과한다.
...
**4. `analyze` 는 Claude 호출을 락 밖에서 기다린다.** 계획의 코드는 세션 락을 쥔 채
호출을 기다렸다. 키가 설정되면 그 몇 초 동안 같은 세션의 `/api/state` 폴링이 전부 막혀
시장이 멈춘다. 스펙 2절은 "분석 중에도 시장은 계속 움직인다. 기다리는 몇 초가 분석의
실질 비용이다" 라고 정하므로, 시장이 멈추면 분석이 공짜가 되고 게임의 유일한 희소 자원이
값을 잃는다. 성능 문제가 아니라 규칙 위반이다. Task 7 리뷰에서 발견했다.

**5. `engine.advance` 는 기여 가능한 계획만 추린다.** `sess.plans` 는 보충마다 20건씩 늘고
가지치기되지 않는데 `advance` 는 매 tick 전체를 훑었다. 폴링당 비용이 세션 길이에 비례해
커지고, 스펙이 일급으로 다루는 "탭을 비웠다 돌아오는" 시나리오가 한 요청에서
ticks x plans 의 CPU 작업이 되어 이벤트 루프를 붙잡아 다른 플레이어까지 멈춘다.
난수는 종목 루프에서만 쓰이므로 필터는 가격을 바꾸지 않는다. Task 7 리뷰에서 발견했다.

**6. 뉴스 어댑터의 실패 경로가 다섯이 아니라 여섯이다.** 스펙과 계획은 클라이언트 없음·
...
**7. 폴백의 격리 가드를 결정론적 2종으로 보강했다.** 브리프의
...
**8. `start_grind` 가 먼저 정산한다.** 스펙은 노가다의 상태 전이를 이 수준까지 규정하지
...
**9. 가격 상태가 로그가격이 아니라 누적 로그수익을 저장한다.** 스펙 3절의 공식은
...
**10. `POLL_INTERVAL_MS` 와 `SPARKLINE_TICKS` 를 `config.py` 에 넣지 않았다.** 둘 다 프론트엔드 전용 수치다. 프론트 계획에서 추가한다.
