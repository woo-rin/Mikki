...
import type { BoardPost, TradeRow } from '../api/types'
import {
  type FeedItem, appendPosts, appendTrades, attachAnalysis, upsertNews,
} from './merge'
...
export const useDerivedStore = create<DerivedState>((set) => ({
  ...
  reset: () =>
    set({ feed: [], trades: [], posts: [], valuations: {}, sessionId: null }),
}))
